...
public abstract class Item 
{
    ...
    public String getEndTm() 
    {
        if( end == 0)
        {
            return TimeUtil.getDiff( App.getStartTm());
        } 
        else 
        {
            return TimeUtil.getDiff( App.getStartTm(), end);
        }
    }
    ...
    {
        return TimeUtil.getDiff( App.getStartTm(), start);
    }
   ...
   protected void setItemAttributes(Element element) throws Exception 
   {
        ...
        if( element.hasAttribute( App.ATT_RESET ))
        {
            setResetFlag( element.getAttribute( App.ATT_RESET ));
        }

        if( element.hasAttribute( App.ATT_RESET ))
        {
            setResetSleep( element.getAttribute( App.ATT_RESET_SLEEP ));
        }

        if( element.hasAttribute( App.ATT_VALIDATE ) )
        {
            setValidatePattern( element.getAttribute( App.ATT_VALIDATE ) );
        }

        if( element.hasAttribute( App.ATT_IS_LIKE ) )
        {
            setMatchResult( element.getAttribute( App.ATT_IS_LIKE ) );
        }

        if( element.hasAttribute( App.ATT_TASK_FILTER ) )
        {
            setTaskFilter( element.getAttribute( App.ATT_TASK_FILTER ) );
        }
       ...
       // }
        ...
        if( element.hasAttribute( App.ATT_UNTIL_ISSET ) )
        {
            setUntilIsSet( element.getAttribute( App.ATT_UNTIL_ISSET ) );
        }

        if( element.hasAttribute( App.ATT_ON_ERROR_SET ) )
        {
            setOnErrorProperty( element.getAttribute( App.ATT_ON_ERROR_SET ) );
        }
        ...
        if( element.hasAttribute( App.ATT_MIN_FIRST))  
        {
            setMinFirst( element.getAttribute(App.ATT_MIN_FIRST));  
        }
        
        if( element.hasAttribute(App.ATT_MAX_FIRST))     
        {
            setMaxFirst( element.getAttribute( App.ATT_MAX_FIRST));  
        }
        if( element.hasAttribute(App.ATT_MIN_MEAN))                  
        {
            setMinMean( element.getAttribute( App.ATT_MIN_MEAN));  
        }
        
        if( element.hasAttribute(App.ATT_MAX_MEAN))              
        {
            setMaxMean( element.getAttribute( App.ATT_MAX_MEAN));
        }
        if( element.hasAttribute(App.ATT_MIN_MODE))                
        {
            setMinMode( element.getAttribute( App.ATT_MIN_MODE));  
        }
        
        if( element.hasAttribute(App.ATT_MAX_MODE))            
        {
            setMaxMode( element.getAttribute(App.ATT_MAX_MODE));      
        }
        if( element.hasAttribute(App.ATT_MIN_MEDIAN))            
        {
            setMinMedian( element.getAttribute( App.ATT_MIN_MEDIAN));  
        }
        
        if( element.hasAttribute(App.ATT_MAX_MEDIAN))               
        {
            setMaxMedian( element.getAttribute( App.ATT_MAX_MEDIAN));  
        }
        
        if( element.hasAttribute( App.ATT_LOOP_FREQUENCY))
        {
            setLoopFrequency( element.getAttribute( App.ATT_LOOP_FREQUENCY));
        }
