...
public class App extends AppCmdLine
{
    ...
    public void handleArgs( Properties p ) throws Exception
    {
        ...
        String configFile = p.getProperty( "-f");
        ...
        {
            throw new IllegalArgumentException("Can not read: " + tmpFile);
        }
        
        String data=FileUtil.readFile(tmpFile).trim();
        if( data.startsWith("{") && data.endsWith("}"))
        {
            confJSON=new JSONObject(data);
            
            xmlFile=confJSON.getString("file");
            Object email = confJSON.opt("email");
            if( email instanceof String)
            {
                emailList=(String)email;
            }
            else if( email instanceof JSONObject)
            {
                emailList=((JSONObject)email).optString("to", emailList);
            }
            
            remoteURL=confJSON.optString("URL", remoteURL);
        }
        else
        {
            xmlFile=configFile;
...
//        Properties props = new Properties();
        ...
        String fromEmail="benmark@stsoftware.com.au";
        if( confJSON!=null)
        {
            if( confJSON.has("email"))
            {
                Object email = confJSON.get( "email");
                
                if( email instanceof JSONObject)
                {
                    fromEmail=((JSONObject) email).optString("from", fromEmail);
                    JSONObject emailJSON=(JSONObject)email;
                    
                    if( emailJSON.has("smtp"))
                    {
                        JSONObject smtpJSON=emailJSON.getJSONObject("smtp");
                        
                        Properties properties = System.getProperties();

                        if( smtpJSON.optBoolean("starttls"))
                        {
                            properties.setProperty("mail.smtp.starttls.enable", "true");
                        }
                        else if( smtpJSON.optBoolean("ssl"))
                        {
                            properties.setProperty("mail.smtp.ssl.enable", "true");
                        }

                        if( smtpJSON.has("host"))
                        {
                            properties.setProperty("mail.smtp.host", smtpJSON.getString("host"));
                        }

                        if( smtpJSON.has("port"))
                        {
                            properties.setProperty("mail.smtp.port", smtpJSON.getString("port"));
                        }

                        if( smtpJSON.has("user"))
                        {
                            properties.setProperty("mail.smtp.user", smtpJSON.getString("user"));
                        }   

                        if( smtpJSON.has("password"))
                        {
                            properties.setProperty("mail.smtp.password", smtpJSON.getString("password"));
                        }
                    }
                }
            }
//             && confJSON.has("smtp")
//            JSONObject smtpJSON = confJSON.getJSONObject("smtp");
            
        }
        
        session = EmailUtil.makeSessionSMTP();
        ...
        msg.setFrom( new InternetAddress( fromEmail ) );//#NOSYNC
